...
FT81x ft81x(PIN_LCD_CS);

// #include "telemetry.h"


// void ft81x_touch_read(lv_indev_drv_t * drv, lv_indev_data_t*data)
// {
//     if( ft81x_process_touch(&data->point.x, &data->point.y) == true) {
...
void GUI::init() {
    ...
    // sprintf(vertice_text_buffer, "%d vertices", NUM_VERTICES);
        ...
        // outputs[PWM_CH_LCD_RESET].set(false);
        // k_busy_wait(500000);

        // outputs[PWM_CH_LCD_RESET].set(true);
        // k_busy_wait(250000);

        LOG_INF("ft81x_reset 5ms\n");
        ...
        while(1) {
            ...
            // const uint64_t uptime = k_uptime_get_32();
    ...
    // indev_drv.read_cb = ft81x_touch_read;
...
bool GUI::begin() {
	...
	lv_obj_align(hello_world_label, LV_ALIGN_CENTER, 0, 250);

    lv_style_init(&style_bg);
    lv_style_set_border_color(&style_bg, lv_palette_main(LV_PALETTE_CYAN));
    ...
    lv_obj_set_size(bar, 400, 40);
    lv_obj_align(bar, LV_ALIGN_CENTER, 0, 200);
    lv_bar_set_range(bar, 0, 100);


    for(uint8_t i=0; i<12; i++) {
        adc_value[i] = lv_label_create(lv_scr_act());
        lv_obj_align(adc_value[i], LV_ALIGN_TOP_LEFT, 20, 20 + 30*i);
    }

    // // lv_label_set_text_fmt(adc_value[0], "5V (V)\t.%.3f", telem.get_value(TELEM_DATA_5V_VOLTAGE));
    // // lv_label_set_text_fmt(adc_value[1], "Battery Voltage (V)\t.%.3f", telem.get_value(TELEM_DATA_BATTERY_VOLTAGE));
    // // lv_label_set_text_fmt(adc_value[2], "Starter Current (A)\t.%.3f", telem.get_value(TELEM_DATA_STARTER_CURRENT));
    // // lv_label_set_text_fmt(adc_value[3], "Regulator Current (A)\t.%.3f", telem.get_value(TELEM_DATA_REGULATOR_CURRENT));
    // // lv_label_set_text_fmt(adc_value[4], "Load Current (A)\t.%.3f", telem.get_value(TELEM_DATA_LOAD_CURRENT));
    // // lv_label_set_text_fmt(adc_value[5], "IS0 Current (A)\t.%.3f", telem.get_value(TELEM_DATA_IS0_CURRENT));
    // // lv_label_set_text_fmt(adc_value[6], "Brake Light Current (A)\t.%.3f", telem.get_value(TELEM_DATA_BRAKE_CURRENT));
    // // lv_label_set_text_fmt(adc_value[7], "IS1 Current (I)\t.%.3f", telem.get_value(TELEM_DATA_IS1_CURRENT));
...
void GUI::drawTelemetry(void) {
    ...
    // // snprintk(buffer, 64, "5V (V)\t\t%.3f", telem.get_value(TELEM_DATA_5V_VOLTAGE));
    // // lv_label_set_text(adc_value[0], buffer);

    // snprintk(buffer, 64, "Battery Voltage (V)\t%.3f\n", telem.get_battery_voltage());
    // lv_label_set_text(adc_value[1], buffer);

    // snprintk(buffer, 64, "Starter Current (A)\t%.3f\n", telem.get_starter_current());
    // lv_label_set_text(adc_value[2], buffer);

    // snprintk(buffer, 64, "Regulator Current (A)\t%.3f\n", telem.get_regulator_current());
    // lv_label_set_text(adc_value[3], buffer);

    // snprintk(buffer, 64, "Load Current (A)\t%.3f\n", telem.get_load_current());
    // lv_label_set_text(adc_value[4], buffer);

    // // snprintk(buffer, 64, "IS0 Current (A)\t%.3f\n", telem.get_value(TELEM_DATA_IS0_CURRENT));
    // // lv_label_set_text(adc_value[5], buffer);

    // // snprintk(buffer, 64, "Brake Light Current (A)\t%.3f\n", telem.get_value(TELEM_DATA_BRAKE_CURRENT));
    // // lv_label_set_text(adc_value[6], buffer);

    // // snprintk(buffer, 64, "IS1 Current (A)\t%.3f\n", telem.get_value(TELEM_DATA_IS1_CURRENT));
    // // lv_label_set_text(adc_value[7], buffer);

    // // // snprintk(buffer, 64, "5V (V)\t\t0x%08x", telem.get_value_raw(TELEM_DATA_5V_VOLTAGE));
    // // // lv_label_set_text(adc_value[TELEM_DATA_5V_VOLTAGE], buffer);

    // // // snprintk(buffer, 64, "Battery Voltage (V)\t0x%08x", telem.get_value_raw(TELEM_DATA_BATTERY_VOLTAGE));
    // // // lv_label_set_text(adc_value[TELEM_DATA_BATTERY_VOLTAGE], buffer);

    // // // snprintk(buffer, 64, "Starter Current (A)\t0x%08X", telem.get_value_raw(TELEM_DATA_STARTER_CURRENT));
    // // // lv_label_set_text(adc_value[TELEM_DATA_STARTER_CURRENT], buffer);

    // // // snprintk(buffer, 64, "Regulator Current (A)\t0x%08X", telem.get_value_raw(TELEM_DATA_REGULATOR_CURRENT));
    // // // lv_label_set_text(adc_value[TELEM_DATA_REGULATOR_CURRENT], buffer);

    // // // snprintk(buffer, 64, "Load Current (A)\t0x%08X", telem.get_value_raw(TELEM_DATA_LOAD_CURRENT));
    // // // lv_label_set_text(adc_value[TELEM_DATA_LOAD_CURRENT], buffer);

    // // // snprintk(buffer, 64, "IS0 Current (A)\t0x%08X", telem.get_value_raw(TELEM_DATA_IS0_CURRENT));
    // // // lv_label_set_text(adc_value[TELEM_DATA_IS0_CURRENT], buffer);

    // // // snprintk(buffer, 64, "Brake Light Current (A)\t0x%08X", telem.get_value_raw(TELEM_DATA_BRAKE_CURRENT));
    // // // lv_label_set_text(adc_value[TELEM_DATA_BRAKE_CURRENT], buffer);

    // // // snprintk(buffer, 64, "IS1 Current (A)\t0x%08X", telem.get_value_raw(TELEM_DATA_IS1_CURRENT));
    // // // lv_label_set_text(adc_value[TELEM_DATA_IS1_CURRENT], buffer);
}
