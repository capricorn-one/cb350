...
void telemetry::init(void) {
    ...
    doc["adc"][TELEM_DATA_STARTER_CURRENT] = 0;
    doc["adc"][TELEM_DATA_REGULATOR_CURRENT] = 0;
    doc["adc"][TELEM_DATA_LOAD_CURRENT] = 0;
    doc["adc"][TELEM_DATA_IS0_CURRENT] = 0;
    doc["adc"][TELEM_DATA_IS1_CURRENT] = 0;
    doc["adc"][TELEM_DATA_IS2_CURRENT] = 0;
    doc["adc"][TELEM_DATA_IS3_CURRENT] = 0;
    doc["adc"][TELEM_DATA_IS4_CURRENT] = 0;
...
void telemetry::update(void) {
    ...
    doc["adc"][TELEM_DATA_STARTER_CURRENT] = adc_raw_data[TELEM_DATA_STARTER_CURRENT].raw;
    ...
    doc["adc"][TELEM_DATA_REGULATOR_CURRENT] = adc_raw_data[TELEM_DATA_REGULATOR_CURRENT].raw;
    ...
    doc["adc"][TELEM_DATA_LOAD_CURRENT] = adc_raw_data[TELEM_DATA_LOAD_CURRENT].raw;
    ...
    doc["adc"][ADC_IS0_CURRENT] = adc_raw_data[ADC_IS0_CURRENT].raw;
    doc["adc"][ADC_IS1_CURRENT] = adc_raw_data[ADC_IS1_CURRENT].raw;
    doc["adc"][ADC_IS2_CURRENT] = adc_raw_data[ADC_IS2_CURRENT].raw;
    doc["adc"][ADC_IS3_CURRENT] = adc_raw_data[ADC_IS3_CURRENT].raw;
    doc["adc"][ADC_IS4_CURRENT] = adc_raw_data[ADC_IS4_CURRENT].raw;
    ...
    doc["v_bat"] = hal_adc_sam_differential_read(ADC_SAM_BATTERY_POS, ADC_SAM_BATTERY_NEG) * 2.624512e-3;
    ...
    doc["i_start"] = hal_adc_sam_analog_read(ADC_SAM_STARTER_CURRENT) * 38.147e-3;
    ...
    doc["temp"] = (hal_adc_sam_internal_temp_read() * 7.62939453125e-5 - 0.688) / 0.00216 + 25.0;
    ...
    doc["vcc"] = hal_adc_sam_vcc_read() * 3.05176e-4;

    serializeJson(doc, LCD_SERIAL);   // send to LCD

    // Debug print usb pretty json
